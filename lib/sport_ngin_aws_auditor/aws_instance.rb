...
module SportNginAwsAuditor
  class AwsInstance
    class << self
      attr_accessor :instances, :reserved_instances, :fully_reserved_permanent, :insufficiently_reserved_permanent, :fully_reserved_temporary, :unutilized_reserved

      # Rolls up instances into a hash of counts
      ...
      def calculate_differences(tag_name)
        ...
        permanent_instance_counts.each do |instance, instance_count|
          ...
          if ri_count > instance_count
            @fully_reserved_permanent[instance] = FractionalCount.new(instance_count, instance_count)
            reserved_instance_counts[instance] -= instance_count
          elsif ri_count == instance_count
            @fully_reserved_permanent[instance] = FractionalCount.new(instance_count, instance_count)
            reserved_instance_counts.delete(instance)
          else
            @insufficiently_reserved_permanent[instance] = FractionalCount.new(ri_count, instance_count)
            reserved_instance_counts.delete(instance)
          end
        end
        ...
        @unutilized_reserved = reserved_instance_counts
      end

      def difference_hash
        {
          fully_reserved_permanent: @fully_reserved_permanent,
          insufficiently_reserved_permanent: @insufficiently_reserved_permanent,
          fully_reserved_temporary: @fully_reserved_temporary,
          insufficiently_reserved_temporary: @insufficiently_reserved_temporary,
          unutilized_reserved: @unutilized_reserved
        }
      end
      ...
      def audit(options)
        ...
        # reserved counts only
        elsif options[:reserved]
          data[:reserved] = instance_count_hash(get_reserved_instances)
        # differences of instances and reserved
        else
          calculate_differences(options[:tag_name])
          data = difference_hash
        end
